...
function getPortfolio() {
    ...
    function formatCurrency(displayAndFormat) {
        switch (displayAndFormat.Currency) {
        case "EUR":
            return "€";
        case "USD":
            return "$";
        case "GBP":
            return "£";
        default:
            return displayAndFormat.Currency;
        }
    }
    ...
    function displayAndFormatValue(displayAndFormat, value) {
        ...
        case "Normal":  // Standard decimal formatting is used with the Decimals field indicating the number of decimals.
            result = formatCurrency(displayAndFormat) + " " + value.toLocaleString(undefined, {minimumFractionDigits: displayAndFormat.Decimals, maximumFractionDigits: displayAndFormat.Decimals});
            break;
        ...
        if (response.ok) {
            response.json().then(function (responseJson) {
                ...
                for (i = 0; i < responseJson.Data.length; i += 1) {
                    ...
                    list += order.Duration.DurationType + " #" + order.OrderId + ": " + order.BuySell + " " + order.Amount + "x " + order.AssetType + " " + order.DisplayAndFormat.Description + " (status " + order.Status + ")" + (
